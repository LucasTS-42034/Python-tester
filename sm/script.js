...
document.getElementById('codeForm').addEventListener('submit', async function(e) {
    ...
    for (let i = 0; i < testCases.length; i++) {
        ...
        const result = await runTest(sourceCode, stdin, expected);
...
async function runTest(sourceCode, stdin, expected) {
    ...
    try {
        ...
        const passed = status === 'Accepted' && stdout.trim() === expected.trim();
